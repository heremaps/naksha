...
For demonstration purposes, we will use an extension that returns a success response for any given request.
...
Start Naksha Service with above custom config, refer steps mentioned at [Naksha_Service - Configuration](../readme.md#configuration).
...
We are using below folder structure for our testing. All extensions should reside under `extensionsRootPath` with the following layout:
...
```text
  ...
  |___extensions
      |___naksha-test-extension
          |___latest-dev.txt  
          |___naksha-test-extension-1.0.0.dev.json
          |___naksha-test-extension-1.0.0-shaded.jar
  
  Contents of latest-dev.txt -> 1.0.0
  Contents of naksha-test-extension-1.0.0.dev.json ->
  {
      "id": "naksha-test-extension",
      "type": "Extension",
      "url": "file:///Users/{{User}}/Desktop/extensions/naksha-test-extension/naksha-test-extension-1.0.0-shaded.jar",
      "version": "1.0.0",
      "properties": {
...
When defining an EventHandler using an extension, use the naming convention: `<env>:<extension-id>`. The `<env>` value ensures that the correct extension is rolled out in the given environment.
...
```json
{
  ...
  "active": true,
  "extensionId": "dev:naksha-test-extension",
  "properties": {
    "skipTagging": false
  }
}
...
2025-09-23 10:38:15.350 -0500 [INFO ]  [main] - lib.extmanager.ExtensionCache (publishIntoCache:111) {streamId=naksha-app} - Whitelist classes in use for extension dev:naksha-test-extension are [java.*, javax.*, com.here.*, jdk.internal.reflect.*, com.sun.*, org.w3c.dom.*, sun.misc.*, org.locationtech.jts.*, org.xml.sax.*, org.slf4j.*] 
2025-09-23 10:38:15.360 -0500 [INFO ]  [main] - lib.extmanager.ExtensionCache (publishIntoCache:153) {streamId=naksha-app} - Extension id=dev:naksha-test-extension, version=1.0.0 is successfully loaded into the cache, using Jar at naksha-test-extension-1.0.0-shaded.jar. 
...
```curl
curl -X 'POST' \
  'http://localhost:8080/hub/spaces/tag_space/features' \
...
When releasing a new version of an extension, Naksha will automatically detect changes and reload the extension if any of the following fields differ from the currently loaded version:
