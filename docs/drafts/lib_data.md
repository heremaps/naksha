...
As the format name indicates, this format is object-oriented. All `JBON` elements are encoded using **units**. Each **unit** always starts with a **header**. The header encodes the type of the unit. There are three general types:

- `JbScalar`: All units that encode a fixed size value (integer, timestamp, ...)
- `JbString`: A special units that encodes UNICODE glyphs
- `JbStruct`: Structures, potentially with child-units (map, array, dict, bytes, tuple, ...)

The `JbScalar` and `JbString` can only be read at ones. The `JbStruct` can be opened, allowing to read the children individually.

The header of all units start with the **lead-in** byte (`JbLeadIn`), which identifies the type of the unit. If **lead-in** byte signals a structure, then it is followed by an optional unsigned integer encoded in 1, 2 or 4 byte (big-endian), storing the size of the payload of the structure. After the size, an optional variant is encoded as 1, 2 or 4 byte integer (big-endian), which allows up to 4 billion subtypes. Following the variant, the payload of the structure follows. Only structures do have a dedicated header size, all other units have always a header size of 1.
...
- **unit-size**: The total outer size of the unit, bytes to seek forward to skip the unit, **unitHeaderSize** plus **unitContentSize**. 
...
- `00`: scalar
  ...
  - `0000_0100`: `Long`, **int8** + 1 byte
  - `0000_0101`: `Long`, **int16** + 2 byte BE payload
  - `0000_0110`: `Long`, **int32** + 4 byte BE payload
  - `0000_0111`: `Long`, **int64** + 8 byte BE payload
  - `0000_1000`: Reserved
  - `0000_1001`: `Double`, **float32** + 4 byte BE payload
  - `0000_1010`: `Double`, **float64** + 8 byte BE payload
  - `0000_1011`: Reserved
  - `0000_1100`: `Long`, Timestamp + 6 byte BE unsigned integer payload
  ...
  - `0000_1110`: Reserved
  - `0000_1111`: Reserved
  - `0001_0000`: `Long`, Local-Index, **uint8** + 1 byte _(biased by 16)_
  - `0001_0001`: `Long`, Local-Index, **uint16** + 2 byte BE _(biased by 16)_
  - `0001_0010`: `Long`, Local-Index, **uint32** + 4 byte BE _(biased by 16)_
  - `0001_0011`: Reserved
  - `0001_0100`: `Long`, Global-Index, **uint8** + 1 byte _(biased by 16)_
  - `0001_0101`: `Long`, Global-Index, **uint16** + 2 byte BE _(biased by 16)_
  - `0001_0110`: `Long`, Global-Index, **uint32** + 4 byte BE _(biased by 16)_
  - `0001_0111`: Reserved
  - `0001_1000`: Reserved
  - `0001_1001`: Reserved
  - `0001_1010`: Reserved
  ...
  - `0001_1110`: Reserved
  - `0001_1111`: Reserved
  - `0010_vvvv`: `Long`, Local-Index, **uint4** (lower 4-bit as unsigned int, 0 to 15)
  - `0011_vvvv`: `Long`, Global-Index, **uint4** (lower 4-bit as unsigned int, 0 to 15)
- `01`: scalar - tiny-encoding
  - `0100_vvvv`: `Long`, **int5** (0 to 15)
  - `0101_vvvv`: `Long`, **int5** (-16 to -1)
  - `0110_vvvv`: `Double`, **float5** (0.0 to 15.0)
  - `0111_vvvv`: `Double`, **float5** (-16.0 to -1.0)
- `10`: string
  - `10vv_vvvv`: `JbString` (vv_vvvv; size 0-60, 61=uint8, 62=uint16, 63=uint32)
  - `1011_1101`: Value 61, followed by 1 byte size _(biased by 60)_
  - `1011_1110`: Value 62, followed by 2 byte _(BE)_ size _(biased by 60)_
  - `1011_1111`: Value 63, followed by 4 byte _(BE)_ size _(biased by 60)_
- `11`: struct
  - General syntax is: `11vv_ttss`
    - `vv`: `Integer`, **Variant** (0=no-variant, 1=int8, 2=int16, 3=int32)
      - To simplify implementation, the **no-variant** is represented at runtime by the variant value of `-1` _(aka `0xffffffff`)_.
    - `tt`: `Integer`, Structure-type.
    - `ss`: `Integer`, size: 0=empty, 1=uint8, 2=uint16, 3=uint32
      - If not empty, followed by a one, two, or four byte unsigned integer, storing the payload size, big-endian encoded.
  - Types without variant _(`vv` is always `00`):
    - `1100_00ss`: `JbArray`, Array _(static methods plus instance to use as List)_
    - `1100_01ss`: `JbMap`, Map _(static methods plus instance to use as Map)_
    - `1100_10ss`: `JbDict`, Dictionary _(static methods, plus instance to as Map and List)_
    - `1100_11ss`: `JbBytes`, Binary _(static methods, operates simply on `ByteArray` aka `byte[]`)_
  - Types with variant _(`vv` values `01`, `10`, and `11`):
    - `11vv_00ss`: `JbNaksha`, Naksha-types
      - variant=`0`: `JbNakshaGeometry`, Tiny-WKB encoded GeoJSON geometry
      - variant=`1`: `JbNakshaTuple`, a virtual database row, efficiently encoded
    - `11vv_01ss`: Reserved
    - `11vv_10ss`: Reserved
    - `11vv_11ss`: **Custom**-types for arbitrary structures, use variant to detect concrete type, any custom format can be encoded.
...
### Map (1)
In JBON the keys of a map must be strings. All key-value pairs are sorted by the binary representation of the key. The reason for this is to guarantee, that two equal maps generate the same hash-code, when their binary representation is hashed. Note that the key must always be a **string-reference**, either into the local or global dictionary. The values are embedded, even while they may be references.
