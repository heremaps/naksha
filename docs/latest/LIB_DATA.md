...
The Naksha data model is made to exchange data in the form of objects between different applications, components, services, and storages. The data model is designed to support efficient storage, retrieval, and query of objects. It's optimized to exchange objects serialized into [JSON], [GeoJSON], [protobuf], and [JBON] _(a special binary encoding, highly compact, mostly immutable, and that does not require parsing to read the data, developed specifically for the Naksha data model)_.
...
| Java                 | Type-Emum           | Javascript      | Description                                                                                                                      |
|----------------------|---------------------|-----------------|----------------------------------------------------------------------------------------------------------------------------------|
| `Undefined`          | `UNDEFINED`         | `undefined`     | The undefined type, a singleton in Java.                                                                                         |
| `null`               | `NULL`              | `null`          | A boolean.                                                                                                                       |
| `boolean`            | `BOOL`              | `Boolean`       | A boolean.                                                                                                                       |
| `byte`               | `BYTE`              | `number`        | A 8-bit integer.                                                                                                                 |
| `short`              | `SHORT`             | `number`        | A 16-bit integer.                                                                                                                |
| `int`                | `INT`               | `number`        | A 32-bit integer.                                                                                                                |
| `long`               | `LONG`              | `BigInt`        | A 64-bit integer.                                                                                                                |
| `float`              | `FLOAT`             | `number`        | A 32-bit floating point number.                                                                                                  |
| `double`             | `DOUBLE`            | `number`        | A 64-bit floating point number.                                                                                                  |
| `byte[]`             | `BYTEA`             | `Int8Array`     | A byte-array.                                                                                                                    |
...
| `String`             | `STRING`            | `String`        | A text of [UNICODE] code-points.                                                                                                 |
...
| `Point`              | `POINT`             |                 | `org.locationtech.jts.geom.Point`                                                                                                |
| `MultiPoint`         | `MULTI_POINT`       |                 | `org.locationtech.jts.geom.MultiPoint`                                                                                           |
| `LineString`         | `LINE_STRING`       |                 | `org.locationtech.jts.geom.LineString`                                                                                           |
| `MultiLineString`    | `MULTI_LINE_STRING` |                 | `org.locationtech.jts.geom.MultiLineString`                                                                                      |
| `Polygon`            | `POLYGON`           |                 | `org.locationtech.jts.geom.Polygon`                                                                                              |
| `MultiPolygon`       | `MULTI_POLYGON`     |                 | `org.locationtech.jts.geom.MultiPolygon`                                                                                         |
...
| `JsonProxy`          |                     |                 | A special object that can be linked to a [JsonObject] to extend the object with custom functions.                                |
| `JsonBytes`          |                     |                 | A wrapper for `byte[]`, `short[]`, `int[]`, `long[]`, `float[]`, or `double[]`, granting low level access, implementing [Bytes]. |
| `JsonFeature`        |                     |                 | The mutable [GeoJSON] feature as [JsonProxy] linked to a [JsonMap].                                                              |
|                      |                     |                 |                                                                                                                                  |
| `Bytes`              |                     |                 | An interface for low-level access to primitive arrays _(`byte[]`, `short[]`, `int[]`, `long[]`, `float[]`, or `double[]`)_.      |
| `TupleId`            |                     |                 | The immutable im-memory representation of a unique identifier of a single [tuple]; wraps a [string].                             |
| `TupleNumber`        |                     |                 | The immutable im-memory representation of a unique identifier of a single [tuple]; wraps [Bytes].                                |
| `Version`            |                     |                 | The im-memory representation of a [version] within a [database]; wraps a [Tuple].                                                |
| `Database`           |                     |                 | The im-memory representation of a [database].                                                                                    |
| `Catalog`            |                     |                 | The im-memory representation of a [catalog] within a [database].                                                                 |
| `Collection`         |                     |                 | The im-memory representation of a [collection] within a [catalog].                                                               |
| `Rec`                |                     |                 | The im-memory representation of a [record] within a [collection].                                                                |
| `Tuple`              |                     |                 | The im-memory representation of a [tuple] within a [record].                                                                     |
| `Book`               |                     |                 | The im-memory representation of a [book].                                                                                        |
| `Kind`               |                     |                 | The im-memory representation of a [kind].                                                                                        |
| `Member`             |                     |                 | The im-memory representation of a [member].                                                                                      |
|                      |                     |                 |                                                                                                                                  |
| `JsonEnum`           |                     |                 | A special enumeration implementation that essentially is always encoded as string or number.                                     |
| `JsonTags`           |                     |                 | A set of key-value pairs as [JsonMap] or [JsonList] proxy. The values must be `indexable` values. A list is parsed.              |
...
All data must be represented using these data types to ensure interoperability between different components, storages, and services. The data model supports the basic components to build more complex structures. Actually, this data model provides primitives, arrays, and maps.

Therefore, desiging your own data model using `lib-data` requires mainly proxy creation. Technically, all needed data structures can be modeled using these basic types.

The data model differs mainly between shared immutable binary data, encoded in [JBON], and mutable thread-local data that is represented as [JSON] objects. The immutable binary data is used for caching and internal administration or fast transport between components or services, while the mutable [JSON] objects are used for the application logic _(i.e. where [JTS] geometry need to be processed)_, serialization, and mutation.
...
public enum Action {
  ...
  public static @NotNull Action of(long ordinal) {
    return of((int)ordinal);
  }
}
```
...
If the identifier is not a valid 63-bit unsigned integer, it will generate the _**secondary logical bytes**_ of the `id`, which is actually the UTF-16 code-units in big-endian byte-order. Then it will hash these bytes using a 64-bit [MurMur3] hash, and use the lowest 63-bit of the hash as _record-number_, with the highest bit being set, so that the number becomes negative.

Therefore, a translation between `id` and `record_number` does not require any database access, and can be done locally. However, it means that there is a possibility of hash-collisions, even while it very low. To solve this, conflict resolution can be enabled. When done so, the storage will handle conflicts, when the `id` and `record_number` do not match. When creating a new record with collision handling enabled, the storage will check if there is already a record with the same `record_number`. If there is, it will check if the `id` of the existing record matches the `id` of the new record. If they do not match, and the `record_number` is negative, it will do the following:

- Logically AND the `record_number` with `0x7fff_ffff_ffff_ffff` _(so clear the sign bit)_.
- Keep a copy of the lowest 16-bit _(`var partition = record_number & 0xffff`)_.
- Add 65536 to the `record_number`.
- Clear the lowest 16-bit and the sign bit of the `record_number` _(`record_number &= 0x7fff_ffff_ffff_0000`)_.
- Finally, set the highest bit and the lowest 16 bit _(`record_number = record_number | 0x8000_0000_0000_0000 | partition`)_.

This results in a new `record_number` that has the same lowest 16-bit as the originally hashed value. This guarantees that the record is effectively kept in the same partition as it originally was in. This is important, because clients that do not know that there is a collision, will always look for the same partition, so they will find both objects in it.
...
public class Tuple implements ITuple, ITupleAddress {
  public Tuple(@NotNull DataRecord rec, @NotNull JbonTuple jbon) {
    ...
    this.rec = rec;
    this.jbon = jbon;
    this.tupleNumber = new TupleNumber(
      rec.collection.catalog.db.number,
      rec.collection.catalog.number,
      rec.collection.number,
      rec.number,
      jbon.version()
    );
  }
...
This two-phase-query approach allows as well to use external caches, like [Redis], [S3], or a combination of them.
...
The **historic partitioning** is done by the `next_version` of each [tuple]. All [tuple] with `next_version` being `null`, are located in the _HEAD_ section, and are only distribution partitioned. When a new [tuple] of a [record] is created, the current [tuple] in the _HEAD_ section becomes historic data. It now needs to be moved to history, and `next_version` must be set to the version of the new [tuple]. The [tuple] should be relocated into the _HISTORY_, which is where **historic partitioning** happens. It will stay in _HISTORY_ immutable until being purged. The purging is normally done by deletion of complete historic partitions, which is the reason for this design. Beware that formally the immutability of a [Tuple] slightly broken here, because the `next_version` is modified while moving the [Tuple] into history. However, this is the only exception, and a not significant one for the caches. Actually `next_version` is no reliable field, applications should ignore it. The value can be calculated using the back-references from `prev_version`, starting at _HEAD_.

Now, when deciding in which historic partition a [Tuple] should be located a **partition-key** is needed. To generate the **partition-key** the value from `next_version` is used. For this, the `next_version` is bitwise-ANDed with `0x000F_FFFF_FFFF_FFFF` _(effectively clearing the top 12-bit)_. Then value is shifted right by a configured `shift` amount. The `shift` is configured when creating a [collection] and must stay constant for the whole lifetime of a [collection]. The `shift` defaults to `17`, which means we store 131,072 versions per historic partition (`2^17`). Assuming that ~100 [tuple] are stored per version (sometimes more, sometimes less), this would result in ~13 million [tuple] per historic partition. When ~1000 [tuple] are created per version, then ~130 million [tuple] are stored per historic partition. All [tuple] sharing the same **partition-key** are stored in the same _HISTORY_ partition _(which by itself can be distribution partitioned)_.

## Origin
The `origin` stores a [reference] to the origin of a [record]. When a [record] is modified, normally the _metadata_ of the new [tuple] is not modified by the client. Therefore, the moment the new [tuple] is sent to a storage, the storage can check the [tuple-number] of the [tuple], which will refer to the state the client modified. If this state is located outside the [collection] into which the new [tuple] is stored, the storage automatically fill the `origin` field with a reference to the old state.
...
This logical representation does not need to match the technical one, as it is not normalized. For example, technically there could be a place to store the [record], which could store the `record_number`, `id`, and `created_at` values, while the `database_number`, `catalog_number`, and `collection_number` may be implicitly known by the storage location, and may not have any need to be stored at all. In the logical model this data reduction becomes visible. Storing the same identifier over and over in the database does not make much sense, the same applies for the `created_at` value, which the same and immutable for all tuple. However, how the logical data is stored physically is left to the storage engine.

The `author_ts` must match the `updated_at` of the tuple in which the `author` changed. This can be used to efficiently query for the changes done by a specific user or application, for example to find out when a specific author made changes to a [record]. Automation and jobs will not set the `author` or `author_ts` fields, they will copy the values from the previous tuple into the next one, so that the `author` stays unchanged between state changes, just the `app` changes.

Therefore, the `app` should always represent the identifier of the application that created the tuple, while the `author` should represent the identifier of the author of the record in a certain time span. This is a logical concept that assist for example in the tracking of [KPI](https://en.wikipedia.org/wiki/Performance_indicator).
