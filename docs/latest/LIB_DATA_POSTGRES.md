...
This part of the documentation explains details about the reference implementation of the Naksha Data Abstract, based upon a PostgresQL database.
...
| Column           | Type   | RO  | Modifiers | Description                                                                                                    |
|------------------|--------|-----|-----------|----------------------------------------------------------------------------------------------------------------|
| fn               | int8   | yes | NOT NULL  | Feature number.                                                                                                |
| version          | int8   | yes | NOT NULL  | Feature version.                                                                                               |
| next_version     | int8   | yes |           | Tuple-Number of the next state, `NULL`, if this is the _HEAD_.                                                 |
| prev_version     | int8   | yes |           | Tuple-Number of the previous state, `NULL`, if this is the first _(CREATE)_.                                   |
| base_version     | int8   | no  |           | Tuple-Number of the state that was modified in a merge case.                                                   |
| created_at       | int8   | yes |           | `.properties.xyz.createdAt` - The time when the transaction started (`transaction_timestamp()`).               |
| updated_at       | int8   | yes | NOT NULL  | `.properties.xyz.updatedAt` - The time when the transaction feature was last modified.                         |
| author_ts        | int8   | no  |           | The EPOCH in milliseconds when the author changed, must match any historic `updated_at`.                       |
| ~~hash~~         | int4   | no  |           | `.properties.xyz.hash`.                                                                                        |
| ~~here_tile_id~~ | int4   | no  |           | `.properties.xyz.hereTileId` - HERE binary quad-key level 15 above `geo_ref`.                                  |
| flags            | int4   | yes | NOT NULL  | Bitmask about how geometry and feature are encoded (GZIP, LZ4, ...).                                           |
| cc               | int4   | yes | NOT NULL  | Change count, starting with 1 when created, incremented for every change.                                      |
| id               | text   | no  | NOT NULL  | `.id` - The feature identifier.                                                                                |
| app_id           | text   | no  |           | `.properties.xyz.appId` - Identifier of the app that produced this state.                                      |
| author           | text   | no  |           | `.properties.xyz.author` - Identifier of the logical owner/author of the state _(credit scoring)_.             |
| origin           | text   | no  |           | `.properties.xyz.origin` - The origin of the feature _(tuple-number)_.                                         |
| target           | text   | no  |           | `.properties.xyz.target` - The target when states are joined _(tuple-number)_.                                 |
| ft               | text   | no  |           | `.properties.featureType` - The feature type _(MOM10: `f.momType`)_.                                           |
| tags             | bytea  | no  |           | `.properties.xyz.tags` - Deprecated, we should get people to rather map properties to custom value properties. |
| ref_point        | bytea  | no  |           | `.referencePoint` - The reference point (`ST_Centroid(geo)`, (**set by the sequencer**).                       |
| geo              | bytea  | no  |           | `.geometry` - The geometry of the features modified (**set by the sequencer**).                                |
| feature          | bytea  | no  |           | `.` - The Geo-JSON feature in JBON, except for what was extracted.                                             |
| attachment       | bytea  | no  |           | Not exposed in Geo-JSON.                                                                                       |
| ci0 .. ci9       | int8   | no  |           | `.properties.custom.ci{n}` - Custom integer value _(with conditional index for not `NULL` values).             |
| cf0 .. cf9       | float8 | no  |           | `.properties.custom.cf{n}` - Custom value _(with conditional index for not `NULL` values).                     |
| cs0 .. cs9       | text   | no  |           | `.properties.custom.cs{n}` - Custom string _(with conditional index for not `NULL` values).                    |
...
FROM table t
JOIN lookup l
ON (t.id, t.version) = (l.id, l.version);
```
...
Add support for automatic dictionary training. This is done using `pg_cron` and should run ones a day. It will check all collections and update the statistics. Additionally, it will update the `default_feature` and `default_tags` tags. These are tags that refer to the latest dictionary to be used for encoding a _feature_ or _tags_ [JBON](lib_data.md)'s. So, when encoding new features or tags for a collection, the default behavior should be to read the latest version and encode with this.
