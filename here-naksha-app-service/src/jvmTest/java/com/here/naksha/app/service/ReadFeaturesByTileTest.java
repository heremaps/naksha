  ...
  */
  ...
  private static Stream<Arguments> standardTestParams() {
    return Stream.of(
        ...
        standardTestSpec(
                // for given filter tileId and f.ids
                "tc0827_testGetByTileAndIds",
                TYPE_QUADKEY,
                "1",
                List.of(
                        "f.id=my-custom-id-800-1",
                        "f.id=my-custom-id-800-2,my-custom-id-800-3"
                ),
                "ReadFeatures/ByTile/TC0827_TileWithFIds/feature_response_part.json",
                200,
                false
        )
    );
  ...
  private static Arguments standardTestSpec(final String testDesc,
    ...
    return Arguments.arguments(tileType, tileId, queryParamList, fPathOfExpectedResBody, expectedResCode,
        Named.named(testDesc, strictChecking));
  }
