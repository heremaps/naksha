...
class SearchFeaturesTest extends ApiTest {
  ...
  @Test
  void tc1010_testSearchWithArrayBasedPropPathFilter() throws URISyntaxException, InterruptedException, IOException {
    // Test API : GET /hub/spaces/{spaceId}/search
    // Validate features returned matches with Property filter (even when Tags filter not supplied)
    // Given: search query
    ...
    final String expectedBodyPart = loadFileOrFail("ReadFeatures/Search/TC1010_searchWithArrayBasedPropPathFilter/search_response.json");
    final String streamId = UUID.randomUUID().toString();
