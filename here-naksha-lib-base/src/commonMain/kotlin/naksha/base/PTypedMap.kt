...
@JsExport
open class PTypedMap<K : Any, V : Any>(
    map: BaseMap,
    /** The Kotlin class of the map keys. */
    val keyKlass: KClass<out K>,
    /** The Kotlin class of the map values. */
    val valueKlass: KClass<out V>
) : Proxy(map), MutableMap<K, V?> {

    /**
     *
     */
    protected val jsMap: BaseMap
        get() = baseObject as BaseMap
    ...
    fun <T : Any> getOr(key: K, alternative: T): T {
        ...
        val value = box(raw, Base.klassOf(alternative))
    ...
    fun <T : Any> getOrSet(key: K, alternative: T): T {
        val data = platformObject()
        val raw = map_get(data, key)
        var value = box(raw, Base.klassOf(alternative))
        if (value == null) {
            value = alternative
     ...
     * @return the value.
     */
    fun <T : Any, KEY : K, SELF : PTypedMap<K, V>> getOrInit(
        key: KEY,
        klass: KClass<out T>,
     ...
     * @return The value.
     */
    fun <T : Any, KEY : K, SELF : PTypedMap<K, V>> getOrCreate(
        key: KEY,
        klass: KClass<out T>,
        ...
        val value: T?
        if (Base.isAssignable(BaseEnum::class, klass)) {
            @Suppress("UNCHECKED_CAST")
            value = BaseEnum.get(raw, klass as KClass<out BaseEnum>) as T
        } else {
    ...
    /**
     * Returns the value of the given key as [Id]. If the value is a string or valid number, convert it into an [Id] instance using the algorithm for the given type.
     ...
     * @return the value of the as [Id] or `null`, if the value is no valid [Id].
     * @since 3.0
     */
    @JvmOverloads
    fun getId(key: K, update: Boolean = true): Id? {
        val v = getRaw(key)
        if (v is Id) return v
        val id: Id? = Id.fromValue(v)
        if (id != null && update) setRaw(id, v)
        return id
    }

    /**
     * Test if the value of the given key is an [Id].
     * @param key the key to test.
     * @return _true_ if the key exists and is either [Id] or `String`; _false_ otherwise.
     * @since 3.0
     */
    fun hasIdValue(key: K): Boolean {
        val v = getRaw(key)
        return v is Id || v is String
    }
    ...
    class Entry<K : Any, V : Any>(
        override val key: K,
        initialValue: V?,
        private val owner: PTypedMap<K, V>
    ) : MutableEntry<K, V?> {
