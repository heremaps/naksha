...
public class DefaultStorageHandler extends AbstractEventHandler {
  ...
  public @NotNull Response process(@NotNull IEvent event) {
    ...
    try {
      ...
      NakshaCollection collectionForCreation = CollectionIndexPolicy.normalizeForHubCreation(collection, collectionId, mapId);
      OperationData operationData = new OperationData(sessionOptions, storageImpl, mapId, collectionId, collectionForCreation, request);
      ...
      if (isOnlyWriteCollections(wr)) {
        wr.getWrites().forEach(write -> {
          if (write.getFeature() instanceof NakshaCollection) {
            NakshaCollection collectionFromRequest = (NakshaCollection) write.getFeature();
            write.setFeature(CollectionIndexPolicy.normalizeForHubCreation(collectionFromRequest, collectionId, mapId));
          }
        });
      }
  ...
  private Response createXyzCollection(
    ...
    return storageImpl.useWriteSession(sessionOptions, writer -> {
      final NakshaCollection collectionToCreate = CollectionIndexPolicy.normalizeForHubCreation(collection, collectionId, mapId);
      final Response result = writer.execute(createWriteCollectionsRequest(collectionToCreate));
      if (result instanceof SuccessResponse) {
        writer.commit();
        return result;
      } else if (result instanceof ErrorResponse) {
