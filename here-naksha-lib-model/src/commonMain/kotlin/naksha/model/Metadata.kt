...
/**
 * The on heap representation of the metadata of a [Row], generated by the storage, read-only for the client.
 *
 * Every row in the storage does have a unique [row identifier][RowNumber], that is made out of [version], [uid], and the partition number read form [flags].
 */
@JsExport
data class Metadata(
    ...
    private var rowNumber: RowNumber? = null
    ...
    fun rowId(): RowNumber {
        var i = rowNumber
        if (i == null) {
            i = RowNumber(storeNumber, Version(version), uid)
            rowNumber = i
        }
        return i
    }
    ...
    override fun toString(): String = "$id:$rowNumber"
