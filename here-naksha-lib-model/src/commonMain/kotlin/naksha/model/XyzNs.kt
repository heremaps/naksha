...
class XyzNs : AnyObject() {
    ...
    companion object XyzNs_C {
        ...
        private val _ACTION = NotNullEnum<XyzNs, Action>(Action.TYPE) { _, _ -> Action.CREATE }
        private val _OPERATION = NotNullEnum<XyzNs, Operation>(Operation.TYPE) { _, _ -> Operation.CREATE }
        ...
        fun fromMetadata(meta: Metadata): XyzNs {
            ...
            return AnyObject().apply {
                set(UUID, guid.toString())
                if (next_tn != null) set(NUUID, Guid(meta.id, next_tn).toString())
                if (prev_tn != null) set(PUUID, Guid(meta.id, prev_tn).toString())
                if (base_tn != null) set(MUUID, Guid(meta.id, base_tn).toString())
                if (meta.createdAt != meta.updatedAt) set(CREATED_AT, meta.createdAt)
                if (meta.authorTs != meta.updatedAt) set(AUTHOR_TS, meta.authorTs)
                set(UPDATED_AT, meta.updatedAt)
                set(CHANGE_COUNT, meta.changeCount)
                set(APP_ID, meta.appId)
                if (meta.author != null) set(AUTHOR, meta.author)
                set(FLAGS, meta.flags)
                set(OPERATION, meta.flags.operationEnum().toString())
                set(ACTION, meta.flags.actionEnum().toString())
                set(HASH, meta.hash)
                set(HERE_TILE, meta.hereTile)
                if (meta.origin != null) set(ORIGIN, meta.origin)
                if (meta.target != null) set(TARGET, meta.target)
                if (meta.cv0 != null) set(CV0, meta.cv0)
                if (meta.cv1 != null) set(CV1, meta.cv1)
                if (meta.cv2 != null) set(CV2, meta.cv2)
                if (meta.cv3 != null) set(CV3, meta.cv3)
                if (meta.cs0 != null) set(CS0, meta.cs0)
                if (meta.cs1 != null) set(CS1, meta.cs1)
                if (meta.cs2 != null) set(CS2, meta.cs2)
                if (meta.cs3 != null) set(CS3, meta.cs3)
            }.proxy(TYPE)
        }
    ...
    /**
     * The target of a [join][Operation.JOIN].
     *
     * The value is a [Guid] as defined by **Naksha**, and refers to the outcome of a [join][Operation.JOIN].
     *
