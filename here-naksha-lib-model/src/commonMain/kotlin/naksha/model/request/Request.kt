...
abstract class Request(
    /**
     * true - no feature body will be returned in the response nor as pure byte[] nor as feature object.
     */
    val noFeature: Boolean = false,
    /**
     *  true - no geometry will be returned in the response nor as pure byte[] nor in feature.geo attribute.
     */
    val noGeometry: Boolean = false,
    /**
     * true - no metadata will be returned in the response nor as pure byte[] nor in feature.properties.xyz attribute.
     */
    val noMeta: Boolean = false,
    /**
     * true - no tags will be returned in the response nor as pure byte[] nor in feature.properties.xyz.tags attribute.
     */
     ...
     *
     * The filter lambdas are called in LIFO order (last in, first out). The output each the lambda is used as input for the next one. So, only if all return a valid new row, the last returned row will be added to the response. This means, each filter can modify the row or cause it to be removed from the response.
     *
