@file:Suppress("OPT_IN_USAGE")

package naksha.model.request

import naksha.model.*
import naksha.model.objects.NakshaFeature
import kotlin.js.JsExport
import kotlin.js.JsStatic
import kotlin.jvm.JvmField
import kotlin.jvm.JvmStatic

/**
 * A [tuple][Tuple] as part of some result or intermediate thread local processing.
 *
 * Note that result-tuple are not thread safe, they are helpful to cache individual states of [NakshaFeature] instances, and when there is only an optional demand to physically read all the underlying data. So, a result-tuple basically means that there is a tuple being part of a result-set, but it might not yet have been loaded into memory.
 *
 * Assume for example, there are 500,000 tuples part of a transaction, it is most often not useful to load all of them into memory, but we need at least the meta-information about all of them, so that they are part of the result-set, then normally a process steps through the result-set and stops, when enough have been processed.
 * @since 3.0.0
 */
@JsExport
open class ResultTuple(
    /**
     * The tuple-number.
     * @since 3.0.0
     */
    @JvmField val tupleNumber: TupleNumber,

    /**
     * The [Tuple].
     *
     * Can be _null_, when not yet fetched from the storage, use [ISession.fetchTuples] or when [op] is [PURGED][ExecutedOp.PURGED] or [RETAINED][ExecutedOp.RETAINED].
     * @since 3.0.0
     */
    @JvmField var tuple: Tuple? = NakshaCache[tupleNumber],

    /**
     * The operation that was executed.
     * @since 3.0.0
     */
    @Deprecated("Do we really need this?", level = DeprecationLevel.WARNING)
    @JvmField var op: ExecutedOp = ExecutedOp.fromAction(tupleNumber.flags.action())
) {
    /**
     * Returns the feature-id, if available.
     * @return the feature-id, if available.
     * @since 3.0.0
     */
    fun id() : String? = tuple?.id

    /**
     * Convert the tuple into a feature, and cache the feature.
     *
     * **Beware**: If the returned feature is modified, this will as well modify the cached version in the row.
     * @return the row converted into a feature, cached in this result-row.
     * @since 3.0.0
     */
    var feature: NakshaFeature? = null
        get() {
            if (field == null) {
                val tuple = this.tuple ?: NakshaCache[tupleNumber]
                if (tuple != null) field = tuple.toNakshaFeature()
            }
            return field
        }

    /**
     * Convert the tuple into a feature, bypass the cache.
     *
     * @return a new copy of the tuple converted into a feature.
     * @since 3.0.0
     */
    fun newFeature(): NakshaFeature? = tuple?.toNakshaFeature()

    companion object ResultTuple_C {
        /**
         * Helper mainly for Java to invoke the constructor with defaults for a [TupleNumber]; this method will load the [Tuple] from the [NakshaCache], if it is contained in it.
         * @param tupleNumber the [TupleNumber] for which to create a [ResultTuple].
         * @return the [ResultTuple].
         * @since 3.0.0
         */
        @JvmStatic
        @JsStatic
        fun fromTupleNumber(tupleNumber: TupleNumber): ResultTuple = ResultTuple(tupleNumber)
    }
}