...
import naksha.base.*
import naksha.base.Fnv1a32.int16BE
import naksha.base.Fnv1a32.int8
...
class Static {
    companion object {
        ...
        @JvmStatic
        @JsStatic
        fun partitionNumber(id: String, partitionCount: Int): Int =
            Fnv1a32.string(Fnv1a32.start(), id).absoluteValue % (partitionCount)
         ...
         */
        @JvmStatic
        fun partitionNameForId(id: String, partitionCount: Int): String =
            PARTITION_ID[partitionNumber(id, partitionCount)]
        ...
        fun tableExists(sql: IPlv8Sql, name: String, schemaOid: Int): Boolean {
            val rows = asArray(
                sql.execute(
                    "SELECT oid FROM pg_class WHERE relname = $1 AND relnamespace = $2",
                    arrayOf(name, schemaOid)
                )
            )
...
//        sql.execute(query)
        ...
        private fun collectionAttachTriggers(sql: IPlv8Sql, id: String, schema: String, schemaOid: Int) {
            ...
            var rows = asArray(
                sql.execute(
                    "SELECT tgname FROM pg_trigger WHERE tgname = $1 AND tgrelid = $2",
                    arrayOf(triggerName, schemaOid)
                )
            )
            if (rows.isEmpty()) {
...
DROP TABLE IF EXISTS $hstName CASCADE;"""
        ...
        private fun createPartitionById(
            ...
            val partitionName =
                if (parentName.contains('$')) "${parentName}_p$partString" else "${parentName}\$p$partString"
            val partitionNameQuoted = sql.quoteIdent(partitionName)
            val parentTableNameQuoted = sql.quoteIdent(parentName)
            val query =
                pgTableInfo.CREATE_TABLE + "IF NOT EXISTS $partitionNameQuoted PARTITION OF $parentTableNameQuoted FOR VALUES FROM ($part) TO (${part + 1}) ${pgTableInfo.STORAGE_PARAMS} ${pgTableInfo.TABLESPACE};"
            sql.execute(query)
