...
class SingleCollectionWriter(
    ...
    fun writeCollections(writeRequest: WriteRequest): SuccessResponse {
        ...
        for (op in operations.operations) {
            ...
            when (opType) {
                XYZ_OP_CREATE -> {
                    val newCollection = when (op.write) {
                        is WriteFeature ->op.write.feature.proxy(NakshaCollection::class)
...
//                    )
    ...
    private fun calculateOpToPerform(row: NakshaRequestOp, existingFeature: Row?, collectionConfig: NakshaCollection): Int {
        return if (row.write.op == XYZ_OP_UPSERT) {
            if (existingFeature != null) {
                XYZ_OP_UPDATE
            } else {
                XYZ_OP_CREATE
            }
        } else if (row.write.op == XYZ_OP_DELETE && collectionConfig.autoPurge) {
            XYZ_OP_PURGE
        } else {
            row.write.op
        }
