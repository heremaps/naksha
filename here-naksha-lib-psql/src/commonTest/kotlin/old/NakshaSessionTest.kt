...
//
//class NakshaSessionTest : TestBasics() {
//
//    private val collectionId = "foo_common"
//    private val writeSession = storage.newWriteSession(PsqlTestStorage.context)
//
//    @Test
...
//
//        val op2 = WriteRow(PgStatic.SC_TRANSACTIONS, Row(storage, DEFAULT_FLAGS, writeSession.transaction.id, tags = tagsBytes))
//
...
//        assertEquals(1, writeSession.transaction.collections[collectionId]?.inserted)
//        assertEquals(1, writeSession.transaction.collections[PgStatic.SC_TRANSACTIONS]?.updated)
...
//        val result = writeSession.usePgConnection().asRows(writeSession.usePgConnection().execute("""SELECT EXISTS (
