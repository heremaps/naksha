...
public class ImportQueue extends JobQueue {
    ...
    JDBCImporter.getRDSStatus(job.getTargetConnector())
        .onSuccess(rdsStatus -> {
          ...
          if (rdsStatus.getCurrentMetrics().getCapacityUnits() > Service.get().config.JOB_MAX_RDS_CAPACITY) {
            p.fail("JOB_MAX_RDS_CAPACITY to high " + rdsStatus.getCurrentMetrics().getCapacityUnits() + " >"
                + Service.get().config.JOB_MAX_RDS_CAPACITY);
            return;
          }
          if (rdsStatus.getCurrentMetrics().getCpuLoad() > Service.get().config.JOB_MAX_RDS_CPU_LOAD) {
            p.fail("JOB_MAX_RDS_CPU_LOAD to high " + rdsStatus.getCurrentMetrics().getCpuLoad() + " > "
                + Service.get().config.JOB_MAX_RDS_CPU_LOAD);
            return;
          }
          if (rdsStatus.getCurrentMetrics().getTotalInflightImportBytes() > Service.get().config.JOB_MAX_RDS_INFLIGHT_IMPORT_BYTES) {
            p.fail("JOB_MAX_RDS_INFLIGHT_IMPORT_BYTES to high " + rdsStatus.getCurrentMetrics().getTotalInflightImportBytes() + " > "
                + Service.get().config.JOB_MAX_RDS_INFLIGHT_IMPORT_BYTES);
            return;
          }
          if (rdsStatus.getCurrentMetrics().getTotalRunningIDXQueries() > Service.get().config.JOB_MAX_RDS_MAX_RUNNING_IDX_CREATIONS) {
            p.fail("JOB_MAX_RDS_MAX_RUNNING_IDX_CREATIONS to high " + rdsStatus.getCurrentMetrics().getTotalInflightImportBytes() + " > "
                + Service.get().config.JOB_MAX_RDS_MAX_RUNNING_IDX_CREATIONS);
            return;
          }
          if (rdsStatus.getCurrentMetrics().getTotalRunningImportQueries() > Service.get().config.JOB_MAX_RDS_MAX_RUNNING_IMPORTS) {
            p.fail("JOB_MAX_RDS_MAX_RUNNING_IDX_CREATIONS to high " + rdsStatus.getCurrentMetrics().getTotalRunningImportQueries() + " > "
                + Service.get().config.JOB_MAX_RDS_MAX_RUNNING_IMPORTS);
            return;
          }
          p.complete();
        }).onFailure(f -> {
  ...
  protected void process() throws InterruptedException, CannotDecodeException {
    ...
    final ConcurrentHashMap<Job, Job> queue = getQueue();
    if (!queue.isEmpty()) {
      int i = 0;
      final Enumeration<Job> keysEnum = queue.keys();
      try {
              ...
              .onSuccess(ff -> {
                ...
                switch (importJob.getStatus()) {
                  ...
                  case executed:
                    ...
                    break;
                  default: {
                    logger.info("JOB[{}] is currently '{}' - current Queue-size: {}", importJob.getId(), importJob.getStatus(),
                        queueSize());
                  }
                }
              })
  ...
  protected void prepareJob(Job j) {
    ...
    Service.get().jdbcImporter.prepareImport(defaultSchema, (Import) j)
        .compose(
            f2 -> {
              j.setStatus(Job.Status.prepared);
              return Service.get().jobConfigClient.update(null, j);
            })
        .onFailure(f -> {
  ...
  protected void executeJob(Job j) {
    ...
    for (String key : importObjects.keySet()) {
      ...
      long curFileSize = Long.valueOf(
          importObjects.get(key).isCompressed() ? (importObjects.get(key).getFilesize() * 12) : importObjects.get(key).getFilesize());
      double maxMemInGB = new RDSStatus.Limits(Service.get().rdsLookupCapacity.get(j.getTargetConnector())).getMaxMemInGB();

      logger.info("IMPORT_MEMORY {}/{} = {}% of max", NODE_EXECUTED_IMPORT_MEMORY, (maxMemInGB * 1024 * 1024 * 1024),
          (NODE_EXECUTED_IMPORT_MEMORY / (maxMemInGB * 1024 * 1024 * 1024)));
      ...
      if (NODE_EXECUTED_IMPORT_MEMORY < Service.get().config.JOB_MAX_RDS_INFLIGHT_IMPORT_BYTES) {
        ...
        importFutures.add(
            Service.get().jdbcImporter.executeImport(j.getTargetConnector(), defaultSchema, j.getTargetTable(),
                    Service.get().config.JOBS_S3_BUCKET, importObjects.get(key).getS3Key(), Service.get().config.JOBS_S3_BUCKET_REGION,
                    curFileSize, j.getCsvFormat())
                .onSuccess(result -> {
                      NODE_EXECUTED_IMPORT_MEMORY -= curFileSize;
            ...
            t -> {
              ...
              for (String key : importObjects.keySet()) {
                ...
                if (importObjects.get(key).getStatus() == ImportObject.Status.waiting) {
                  ...
                  j.setStatus(Job.Status.prepared);
                  Service.get().jobConfigClient.update(null, j);
                  return;
                }
              }
  ...
  protected void finalizeJob(Job j) {
    ...
    Service.get().jdbcImporter.finalizeImport(j, getDefaultSchema)
        .onFailure(f -> {
