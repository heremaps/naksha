...
public class BurstAndUpdateThread extends Thread {
  ...
  private synchronized void onConnectorList(AsyncResult<List<Connector>> ar) {
    ...
    for (final RpcClient client : RpcClient.getAllInstances()) {
      ...
      final Connector newConnector = connectorMap.get(oldConnector.id);
      ...
      if (!oldConnector.equalTo(newConnector)) {
        try {
          ...
          logger.info("The incoming connector config for \"{}\" is different from the existing one. Updating the according RPC client.",
              newConnector.id);
  ...
  public void run() {
    ...
    while (true) {
      try {
        ...
        Service.get().connectorConfigClient.getAll(null, this::onConnectorList);
